...
class Principal extends Person {
  ...
  transferStudent(student, principal) {
    // note: we have the student right here. No need to remove it from students and pass it to the other principal
    // on the other hand - we should only transfer the student if he exists. or maybe not...

    // v1 - pass the student
    // if (!this.students.includes(student)) {
    //   return {
    //     type: "error",
    //     message: "cannot transfer student: student not found",
    //   };
    // }

    // const indexOfStudentToTransfer = this.students.indexOf(student);
    // principal.students.push(this.students[indexOfStudentToTransfer]);
    // this.students = this.students.splice(indexOfStudentToTransfer, 1);

    // // v2 - check and transfer student
    // if (!this.students.includes(student)) {
    //   return {
    //     type: "error",
    //     message: "cannot transfer student: student not found",
    ...
    // v3 - don't check - just get it done
    principal.students.push(student);
    this.students = this.students.filter((s) => s !== student);
  }
}
